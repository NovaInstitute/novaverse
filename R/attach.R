...
novaverse_attach <- function() {
  ...
  # novaverse with the package version on the right-hand side
  load_header <- cli::rule(
    left = crayon::bold("Attaching packages"),
    right = paste0("novaverse ", package_version("novaverse"))
  )

  # Return a character string containing the package version for each of novaverse's constituents
...
novaverse_detach <- function() {
  pak <- paste0("package:", c(pkgs, "novaverse"))
  lapply(pak[pak %in% search()], detach, character.only = TRUE)
  invisible()
}

#' List all packages imported by novaverse
#'
#' @export
#'
#' @examples
#' novaverse_packages()
novaverse_packages <- function() {
  # get all imports from novaverse's package description file
  raw <- utils::packageDescription("novaverse")$Imports
